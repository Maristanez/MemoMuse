...
> Voice memo → produced music demo → vinyl on Shopify. Record a rough idea, pick a genre, get a mixed track with vocals and instrumentation, then sell it as a vinyl record.
...
```
                         ┌──────────────────────────────────────────────┐
                         │              Frontend (index.html)           │
                         │  Record audio · Pick genre · Play result    │
...
| **5. Generate audio** | Google Lyria Realtime + ElevenLabs | **Instrumental**: Lyria generates 60s at 48 kHz from the style prompt. **Vocals**: if lyrics detected → ElevenLabs TTS (`eleven_multilingual_v2`); if humming → ElevenLabs Speech-to-Speech preserves melody. Both run in parallel. Falls back to instrumental-only if vocals fail |
| **6. Mix** | pydub | Both tracks normalized to −20 dBFS. Vocals boosted +6 dB, instrumental cut −6 dB. Overlay, trim, export final MP3 |
...
### ElevenLabs — Text-to-Speech + Speech-to-Speech
- **TTS** (`eleven_multilingual_v2`): Synthesizes generated lyrics into vocal audio with custom voice settings (stability, similarity, style)
- **STS** (`eleven_multilingual_sts_v2`): When the user hums instead of singing lyrics, preserves the original melody while cleaning up audio quality
- Configurable voice via `ELEVENLABS_VOICE_ID`
...
- Frontend "Buy as Vinyl" button triggers creation and opens the product page
...
| `POST` | `/generate` | Accepts audio + genre, runs pipeline, returns JSON with `audio_url`, `song_title`, `lyrics`, `mood`, `bpm`, `genre`, `key` |
| `GET` | `/audio/{filename}` | Serves generated MP3 files from `temp/` |
| `POST` | `/api/publish` | Creates a vinyl product on Shopify. Accepts `song_title`, `lyrics`, `genre`, `mood`, `bpm`, `key`. Returns `product_url` |
...
- **Buy as Vinyl** — Creates a real Shopify product and opens the store page
...
ELEVENLABS_API_KEY      # ElevenLabs TTS/STS
...
- **Audio normalization**: Both tracks normalized to −20 dBFS before applying relative gain (+6 dB vocals, −6 dB instrumental) for consistent vocal clarity
